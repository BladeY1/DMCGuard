...
import requests
import config
from config import SSL_KEYS_1,SSL_KEYS_2
...
#ssl_keys = (
 #   "cert/101.43.239.148+2.pem",
 #   "cert/101.43.239.148+2-key.pem",
#)
...
class Message(db.Model):
    ...
    name = db.Column(db.String(20))
    channel = db.Column(db.String(20), default="speaker")
    R = db.Column(db.String(20))
...
def operate():
    ...
    current_time = now.strftime("%H:%M:%S.%f")[:-4]  # 取消注释此行以获取时分秒，精确到小数点后两位的秒
    print("服务器接收时间Current Time =", current_time)
    # 新代码结束
    if session.get("logged_in") != "true":
        return jsonify({"code": 400, "msg": "Please login first"})
    name = session.get("name")
...
if __name__ == "__main__":
    ...
    app.run(
        host="0.0.0.0", port=5003, debug=True, ssl_context=ssl_keys, use_reloader=False
    )
